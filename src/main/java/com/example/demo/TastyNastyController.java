...
public class TastyNastyController {
    ...
    public String showIndex(Model model) {
        ...
        for (Food dish : dishList) {
            dish.setLast5minutes(foodService.last5MinutesResult(dish.getId()));
        }
...
/*
    ...
    public String addNastyVote(HttpServletRequest request) {
        ...
        long foodID = new Long(request.getParameter("id"));
        Nasty tastyNastyVote = new Nasty();
        tastyNastyVote.setTheDish(dishes.findById(foodID).get());
        tastyNastyVote.setVotedAt();
        nastyVotes.save(tastyNastyVote);
    ...
    public @ResponseBody String pizzaVotes() {
        ...
        String tastyNasty = pizza.getTastyVotes().size()>=pizza.getNastyVotes().size()
                ? "tasty" : "nasty";
        return pizza.getDescription() + " has " + pizza.getTastyVotes().size()
                + " tasty votes and " + pizza.getNastyVotes().size()
                + " nasty votes . Thr balance tilts in favour of: " + tastyNasty;
    }
    ...
    public String saveFood(@Valid @ModelAttribute("theDish") Food toSave,
